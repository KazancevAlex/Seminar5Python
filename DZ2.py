...
welcome_text = ('Условие задачи: На столе лежит 2021 конфета. Играют два игрока делая ход друг после друга\n'
                'Первый ход определяется жеребьёвкой. За один ход можно забрать не более чем 28 конфет\n'
                'Все конфеты оппонента достаются сделавшему последний ход. Сколько конфет нужно взять первому игроку,\n'
                'чтобы забрать все конфеты у своего конкурента?\n')
print(welcome_text)

message = ['Выбери число конфет от 1 до 28']
...
def player_vs_player():
    ...
    player_2 = input('Введи имя соперника: ')

    print(f'Начинают {player_1} и {player_2}\n')
    ...
    while candies_total > 0:
        ...
        if players[lucky % 2] == 'Компьютер':
            ...
            while step > max_take or step > candies_total:
                step = int(input(f'\nМуксимальное число конфет {max_take}, попробуй еще раз: '))
        candies_total = candies_total - step
